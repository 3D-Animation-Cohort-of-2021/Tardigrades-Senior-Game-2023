...
{
    public float viewRadus = 10f;
    [Range(0,361)]
    public float viewAngle = 360f;

    public LayerMask obstacleMask;
    public float meshRes = 1f;
    public int edgeResolve = 5;
    public float edgeDstTreshold = 0.5f;
    ...
    EdgeInfo FindEdge(ViewCastInfo minViewCast, ViewCastInfo maxViewCast)
    {
        ...
        for (int i = 0; i < edgeResolve; i++)
        {
            ...
            ViewCastInfo newViewCast = ViewCast(angle);
            bool edgeDstThresholdExceeded = Mathf.Abs(minViewCast.dst - newViewCast.dst) > edgeDstTreshold;
    ...
    ViewCastInfo ViewCast(float globalAngle)
    {
        ...
        if (Physics.Raycast(transform.position, dir, out hit, viewRadus, obstacleMask))
        {
            return new ViewCastInfo(true, hit.point, hit.distance, globalAngle);
        }
        else
        {
            return new ViewCastInfo(false, transform.position + dir * viewRadus, viewRadus, globalAngle);
        }
    }
